...
from triangular_arbitrage.web_security import SecurityManager
...
# Setup logging with clean configuration
...
# Initialize security manager (auth disabled by default for local use)
_api_key = os.getenv("WEB_API_KEY")
_enable_auth = (
    os.getenv("WEB_ENABLE_AUTH", "false").lower() == "true"
)  # Disabled by default
_enable_rate_limit = (
    os.getenv("WEB_ENABLE_RATE_LIMIT", "false").lower() == "true"
)  # Disabled by default
_rate_limit_per_min = int(os.getenv("WEB_RATE_LIMIT_PER_MIN", "60"))

security_manager = SecurityManager(
    api_key=_api_key,
    rate_limit_per_minute=_rate_limit_per_min,
    enable_auth=_enable_auth,
    enable_rate_limit=_enable_rate_limit,
)

# CORS middleware for React frontend - restrict to specific origins
_allowed_origins_str = os.getenv(
    "WEB_ALLOWED_ORIGINS", "http://localhost:3000,http://localhost:8000"
)
_allowed_origins = [origin.strip() for origin in _allowed_origins_str.split(",")]

app.add_middleware(
    CORSMiddleware,
    allow_origins=_allowed_origins,  # Only allow specified domains
...
class ConfigUpdate(BaseModel):
    ...
    min_profit_threshold: Optional[float] = Field(
        None, ge=0.0, le=100.0, description="Minimum profit threshold (0-100%)"
    )
    topn: Optional[int] = Field(
        None, ge=1, le=50, description="Number of top opportunities to show (1-50)"
    )
...
class BotStartRequest(BaseModel):
    """Validated bot start request model."""
...
async def start_bot(
    ...
    _auth: None = Depends(security_manager.verify_api_key),
    _rate_limit: None = Depends(security_manager.check_rate_limit),
):
    """Start the arbitrage bot

    Args:
        request: Validated bot start request
        _auth: Authentication dependency
        _rate_limit: Rate limit dependency
    """
...
async def control_dex(
    request: DexControlRequest,
    _auth: None = Depends(security_manager.verify_api_key),
    _rate_limit: None = Depends(security_manager.check_rate_limit),
):
...
async def run_dex_scanner():
    """
    Background DEX scanner with live blockchain data.
    Uses real pool reserves from on-chain queries.
    ...
    try:
        ...
        if WEB3_AVAILABLE and eth_rpc_url:
            ...
            if not WEB3_AVAILABLE:
                logger.info("Web3.py not installed - cannot scan DEX")
                dex_state.add_log("❌ Web3.py not installed - cannot scan DEX")
                dex_state.running = False
                return
            elif not eth_rpc_url:
                logger.info("No ETH_RPC_URL set - cannot scan DEX")
                dex_state.add_log("❌ No ETH_RPC_URL configured - cannot scan DEX")
                dex_state.running = False
                return

        # Load DEX config - default to BSC (best for finding opportunities)
        config_path = os.getenv("DEX_CONFIG_PATH", "configs/dex_bsc_dynamic.yaml")
        try:
            dex_config = load_config(config_path)
            logger.info(f"Loaded DEX config from {config_path}")
            dex_state.add_log(f"📋 Loaded config: {config_path}")
        except (ConfigError, FileNotFoundError) as e:
            logger.error(f"Failed to load DEX config: {e}")
            dex_state.add_log(f"❌ Config error: {str(e)}")
            dex_state.running = False
            return

        # Initialize DexRunner
        try:
            runner = DexRunner(dex_config, quiet=True)
            runner.connect()
            runner.build_token_maps()
            runner.fetch_pools()
            dex_state.pools_loaded = len(runner.pools)
            logger.info(f"DexRunner initialized with {dex_state.pools_loaded} pools")
            dex_state.add_log(f"✅ Loaded {dex_state.pools_loaded} DEX pools")
        except Exception as e:
            logger.error(f"Failed to initialize DexRunner: {e}")
            dex_state.add_log(f"❌ Scanner init failed: {str(e)}")
            dex_state.running = False
            return
        ...
        while dex_state.running:
            ...
            # Scan for real opportunities using DexRunner
            try:
                # Run async scan for live pool data
                arb_rows = await runner.scan_async()

                # Convert ArbRow results to DexOpportunity format
                for row in arb_rows:
                    # Extract path tokens from cycle string
                    # row.cycle format: "USDC -> WETH (uniswap_v2) -> USDC (sushiswap)"
                    cycle_parts = row.cycle.split(" -> ")
                    path_tokens = []

                    for part in cycle_parts:
                        # Extract token before parentheses or use whole part
                        token = (
                            part.split("(")[0].strip() if "(" in part else part.strip()
                        )
                        # Add token if it's new or it's the closing token
                        if not path_tokens or token != path_tokens[-1]:
                            path_tokens.append(token)

                    # Ensure path closes (last token = first token)
                    if path_tokens and path_tokens[-1] != path_tokens[0]:
                        path_tokens.append(path_tokens[0])

                    # Create basic leg structure from available data
                    # Note: DexRunner ArbRow doesn't have detailed leg info
                    # In production, enhance DexRunner to return per-leg details
                    legs = [
                        {
                            "pair": row.pair,
                            "side": "buy",  # Simplified - would need actual direction
                            "price": 0.0,
                            "liq_usd": 50000.0,  # Placeholder
                            "slip_bps_est": runner.config.slippage_pct * 100,
                            "dex": row.dexA,
                            "type": "v2",
                        ...
                        },
                        {
                            ...
                            "type": "v2",
                            "pool": "0x0000000000000000000000000000000000000000",
                            "token_in": path_tokens[1]
                            if len(path_tokens) > 1
                            else "UNKNOWN",
                            "token_out": path_tokens[0]
                            if len(path_tokens) > 0
                            else "UNKNOWN",
                        },
                    ]

                    # Compute costs for the route
                    ...
                    # Use breakdown from opportunity math
                    breakdown = compute_opportunity_breakdown(
                        gross_bps=row.gross_pct * 100,
                        fee_bps=costs["fee_bps"],
                        safety_bps=runner.config.slippage_pct * 100,
                        gas_usd=(costs["gas_bps"] / 10000.0) * size_usd,
                        trade_amount_usd=size_usd,
                    )

                    # Create DexOpportunity from real data
                    opp = DexOpportunity(
                        id=f"dexop_{scan_count}_{row.pair}_{row.dexA}_{row.dexB}",
                        path=path_tokens,
                        gross_bps=row.gross_pct * 100,
                        gross_pct=row.gross_pct,
                        fee_bps=costs["fee_bps"],
                        fee_pct=costs["fee_bps"] / 100,
                        slip_bps=runner.config.slippage_pct * 100,
                        slip_pct=runner.config.slippage_pct,
                        gas_bps=costs["gas_bps"],
                        gas_pct=costs["gas_bps"] / 100,
                        net_bps=row.net_pct * 100,
                        net_pct=row.net_pct,
                        size_usd=size_usd,
                        legs=legs,
                        ts=time.time(),
                    ...
                    if dex_state.decision_engine and row.net_pct >= (
                        dex_state.config["min_profit_threshold_bps"] / 100.0
                    ):
                        decision = dex_state.decision_engine.evaluate_opportunity(
                            gross_pct=opp.gross_bps / 100.0,
                            fees_pct=opp.fee_bps / 100.0,
                        ...
                        if decision.action == "EXECUTE":
                            ...
                            # Keep equity chart alive
                            dex_state.add_equity_point(current_equity, cumulative_pnl)

            except Exception as e:
                logger.error(f"Scan error: {e}", exc_info=True)
                dex_state.add_log(f"⚠️ Scan error: {str(e)}")
