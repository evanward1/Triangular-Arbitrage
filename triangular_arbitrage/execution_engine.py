        ...
        async with self._lock:
            ...
            for cycle_id, cycle_info in self.cache.items():
                if cycle_info.state in {CycleState.COMPLETED, CycleState.FAILED}:
                    if cycle_info.end_time and current_time - cycle_info.end_time > max_age_seconds:
...
class StateManager:
    ...
    async def initialize(self):
        ...
        async with self._lock:
            ...
            if self.enable_cache:
                ...
                logger.info(f"StateManager initialized with {self.pool_size} connections and caching enabled")
            else:
                logger.info(f"StateManager initialized with {self.pool_size} connections (no caching)")
        ...
        async with self.get_connection() as conn:
            ...
            try:
                ...
                ''', (
                    ...
                    cycle_info.state.value if isinstance(cycle_info.state, CycleState) else cycle_info.state,
    ...
    async def save_order(self, cycle_id: str, order_info: OrderInfo):
        ...
        async with self.get_connection() as conn:
            ...
            ''', (
                ...
                order_info.amount,
                order_info.price,
                order_info.state.value if isinstance(order_info.state, OrderState) else order_info.state,
    ...
    async def update_order_state(self, order_id: str, new_state: OrderState,
        ...
        async with self.get_connection() as conn:
            ...
            params = [new_state.value if isinstance(new_state, OrderState) else new_state, time.time()]
    ...
    async def get_active_cycles(self, strategy_name: Optional[str] = None) -> List[CycleInfo]:
    ...
    async def _get_active_cycles_from_db(self, strategy_name: Optional[str] = None) -> List[CycleInfo]:
...
class OrderManager:
    ...
    def __init__(self, exchange, config: Dict[str, Any]):
        ...
        self.initial_check_delay = self.monitor_config.get('initial_delay_ms', 100) / 1000.0
        ...
        self.rapid_check_threshold = self.monitor_config.get('rapid_check_threshold_ms', 2000) / 1000.0
        self.rapid_check_interval = self.monitor_config.get('rapid_check_interval_ms', 50) / 1000.0

        # Rate limit awareness
        self.rate_limit_buffer = self.monitor_config.get('rate_limit_buffer', 0.8)  # Use 80% of rate limit
        self.min_request_interval = self.monitor_config.get('min_request_interval_ms', 50) / 1000.0
        ...
        while time.time() - start_time < timeout:
            ...
            if status in ['closed', 'filled']:
                ...
                if self.order_config.get('allow_partial_fills', False):
                    ...
                    min_fill_percentage = self.order_config.get('min_partial_fill_percentage', 0.95)
    ...
    def _cache_order_status(self, cache_key: str, order_data: Dict):
        ...
        if len(self.order_cache) > 100:
            current_time = time.time()
            expired_keys = [
                k for k, (t, _) in self.order_cache.items()
    ...
    async def _enforce_rate_limit(self):
        ...
        if len(self.api_call_timestamps) >= 10:
            ...
            if time_window > 0:
                ...
                exchange_rate_limit = getattr(self.exchange, 'rateLimit', 50) / 1000  # Convert to requests/second

                # If we're approaching rate limit, add delay
                if request_rate > exchange_rate_limit * self.rate_limit_buffer:
                    delay = 1.0 / (exchange_rate_limit * self.rate_limit_buffer) - time_window / 10
                    if delay > 0:
...
class StrategyExecutionEngine:
    ...
    def __init__(self, exchange, strategy_config: Dict[str, Any], state_manager: Optional[StateManager] = None):
        self.exchange = exchange
        ...
        self.enable_risk_controls = RISK_CONTROLS_AVAILABLE and (
            self.max_leg_latency_ms is not None or
            strategy_config.get('risk_controls', {}).get('enable_slippage_checks', True)
        )
        ...
        use_enhanced = strategy_config.get('panic_sell', {}).get('use_enhanced_routing', True)
        if ENHANCED_RECOVERY_AVAILABLE and use_enhanced:
            self.recovery_manager = EnhancedFailureRecoveryManager(exchange, strategy_config)
    ...
    async def initialize(self):
        ...
        if self._using_enhanced_recovery and hasattr(self.recovery_manager, 'initialize'):
    ...
    async def _check_risk_controls(self, skip_cycle_count: bool = False) -> bool:
        ...
        if not skip_cycle_count:
            active_cycles = await self.state_manager.get_active_cycles(self.config['name'])
    ...
    async def _execute_cycle_trades(self, cycle_info: CycleInfo) -> bool:
        ...
        if self.risk_control_manager and self.risk_control_manager.is_cycle_in_cooldown(cycle_key):
            cooldown_remaining = self.risk_control_manager.get_cycle_cooldown_remaining(cycle_key)
            cycle_info.error_message = (
        ...
        for i in range(cycle_info.current_step, len(trade_path) - 1):
            ...
            try:
                ...
                if self.risk_control_manager and leg_start_time:
                    latency_measurement, latency_violated = self.risk_control_manager.end_leg_timing(
                ...
                if self.risk_control_manager and order_info.average_price > 0:
                    slippage_measurement, slippage_violated = self.risk_control_manager.track_slippage(
                        i,
                        market_symbol,
                        expected_price,
                        order_info.average_price,
    ...
    async def _handle_panic_sell(self, cycle_info: CycleInfo):
        ...
        if len(panic_result) == 4:
            ...
            cycle_info.metadata['panic_sell_path'] = [step.input_currency for step in execution_steps] + [final_currency] if execution_steps else []
        else:
        ...
        try:
            ...
            for cycle_info in active_cycles:
                ...
                if recovery_action == 'resume':
                    ...
                    await self._mark_cycle_failed(cycle_info, "Unrecoverable state detected")
            ...
            for cycle_info in cycles_to_panic_sell:
                try:
                    ...
                    recovery_stats['errors'].append(f"Panic sell failed: {cycle_info.id}")
            ...
            for cycle_info in cycles_to_validate:
                try:
                    ...
                    if is_valid:
                        ...
                        await self._mark_cycle_failed(cycle_info, "State validation failed")
                        recovery_stats['marked_failed'] += 1
                except Exception as e:
                    logger.error(f"Validation failed for cycle {cycle_info.id}: {e}")
                    recovery_stats['errors'].append(f"Validation failed: {cycle_info.id}")

            # Step 8: Resume recoverable cycles
            for cycle_info in cycles_to_resume:
                try:
                    logger.info(f"Resuming cycle {cycle_info.id} from step {cycle_info.current_step}")
                    ...
                    try:
                        ...
                        logger.error(f"Panic sell also failed for {cycle_info.id}: {panic_e}")
                        await self._mark_cycle_failed(cycle_info, f"Recovery and panic sell failed: {e}")
                        recovery_stats['marked_failed'] += 1
            ...
            logger.info(f"Successfully recovered: {recovery_stats['successfully_recovered']}")
    ...
    async def _validate_database_integrity(self):
        ...
        async with self.state_manager.get_connection() as conn:
            ...
            if orphaned_orders > 0:
                logger.warning(f"Found {orphaned_orders} orphaned orders, cleaning up...")
                await conn.execute('''
    ...
    async def _analyze_cycle_for_recovery(self, cycle_info: CycleInfo) -> str:
        ...
        if cycle_age > max_cycle_age:
            logger.warning(f"Cycle {cycle_info.id} is too old ({cycle_age:.0f}s), marking for panic sell")
            return 'panic_sell'
            ...
            if cycle_info.orders:
                ...
                if last_order.state in [OrderState.PENDING, OrderState.PLACED]:
                    ...
                    if order_age > 300:  # 5 minutes
                        logger.warning(f"Cycle {cycle_info.id} has stale order, needs validation")
                        return 'validate'
            return 'resume'
    ...
    async def _validate_cycle_state(self, cycle_info: CycleInfo) -> bool:
        ...
        try:
            ...
            if cycle_info.current_step >= len(trade_path) - 1:
                logger.error(f"Cycle {cycle_info.id} already at final step but not completed")
                return False
            ...
            if market_symbol_forward not in markets and market_symbol_backward not in markets:
                logger.error(f"No market found for {from_currency} -> {to_currency}")
                return False

            # Check balance is sufficient
            if cycle_info.current_amount <= 0:
                logger.error(f"Cycle {cycle_info.id} has invalid amount: {cycle_info.current_amount}")
                return False
            ...
            try:
                ...
                if status in ['closed', 'filled']:
                    ...
                    if last_order.side == 'buy':
                        ...
                        cycle_info.current_amount = last_order.filled_amount * last_order.average_price
                ...
                try:
                    await self.exchange.cancel_order(last_order.id, last_order.market_symbol)
                    last_order.state = OrderState.CANCELLED
                except:
                    pass
