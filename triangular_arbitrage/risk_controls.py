"""
Risk Controls and Position Management for Triangular Arbitrage Trading.

This module implements comprehensive risk management controls including position limits,
drawdown protection, latency monitoring, and kill-switch mechanisms. It provides
real-time risk assessment and automated safeguards to protect trading capital.

Key Features:
    - Position size and exposure limits
    - Real-time drawdown monitoring with kill-switch
    - Trade execution latency tracking and limits
    - Persistent risk state management
    - Emergency liquidation controls
    - Comprehensive risk metrics and logging

Classes:
    LatencyMeasurement: Individual trade latency tracking
    RiskControlManager: Main risk management orchestrator
    PositionTracker: Position size and P&L tracking
...
class LatencyMeasurement:
    """
    Measurement record for trade execution latency tracking.

    Attributes:
        leg_index: Index of the trading leg in the arbitrage cycle (0, 1, or 2)
        market_symbol: Trading pair symbol (e.g., 'BTC/USDT')
        start_time: Timestamp when order placement began (Unix time)
        end_time: Timestamp when order was filled (Unix time)
...
class LatencyMonitor:
    ...
    def end_measurement(
        self, leg_index: int, market_symbol: str, start_time: float, side: str
    ) -> LatencyMeasurement:
        end_time = time.time()
...
class SlippageTracker:
    ...
    def cleanup_expired_cooldowns(self):
        ...
        expired = [
            k
            for k, v in self.cooldown_cycles.items()
...
class RiskControlLogger:
    def __init__(
        self, log_dir: str = "logs/risk_controls", suppression_window: float = 2.0
    ):
    ...
    def _is_duplicate_event(
        self, cycle_id: str, stop_reason: str, timestamp: float
    ) -> bool:
    ...
    def log_violation(self, violation: RiskControlViolation, is_executed: bool = False):
        if not is_executed:
            ...
            if self._is_duplicate_event(
                violation.cycle_id, stop_reason, violation.timestamp
            ):
                return
    ...
    def _log_to_console(self, violation: RiskControlViolation):
        ...
        self.console_logger.warning(
            f"RISK CONTROL VIOLATION: {violation.violation_type}"
        )
        self.console_logger.warning("=" * 80)
        self.console_logger.warning(f"Cycle ID: {violation.cycle_id}")
        self.console_logger.warning(f"Strategy: {violation.strategy_name}")
        self.console_logger.warning(
            f"Trading Pair: {' -> '.join(violation.cycle_path)}"
        )
        self.console_logger.warning(f"Direction: {violation.cycle_direction}")
        self.console_logger.warning(
            f"Threshold Violated: {violation.threshold_violated}"
        )

        if violation.violation_type == "LATENCY_EXCEEDED":
            self.console_logger.warning("\nLatency Details (ms):")
            for i, (latency, leg) in enumerate(
                zip(violation.latencies_ms, violation.leg_details)
            ):
                status = (
                    "VIOLATED"
                    if latency > violation.threshold_violated["max_leg_latency_ms"]
                    else "OK"
                )
                self.console_logger.warning(
            ...
            self.console_logger.warning("\nSlippage Details:")
            for i, (slippage, expected, actual, leg) in enumerate(
                zip(
                ...
                status = (
                    "VIOLATED"
                    if abs(slippage) > violation.threshold_violated["max_slippage_bps"]
                    else "OK"
                )
                self.console_logger.warning(
    ...
    def _update_suppressed_history(
        self, cycle_id: str, stop_reason: str, cached: Dict[str, Any]
    ):
        record = {
            "cycle_id": cycle_id,
    ...
    def get_recent_suppressed(self, limit: int = 10) -> List[Dict[str, Any]]:
        with self._cache_lock:
            sorted_history = sorted(
                self._suppressed_history, key=lambda x: x["last_seen"], reverse=True
            )
    ...
    def get_suppression_summary(self, window_seconds: int = 300) -> Dict[str, Any]:
        with self._cache_lock:
            ...
            recent_events = [
                event
                for event in self._suppressed_history
            ...
            suppression_rate = (
                (total_suppressed / total_events * 100) if total_events > 0 else 0.0
            )

            sorted_events = sorted(
                recent_events, key=lambda x: x["duplicate_count"], reverse=True
            )
            top_pairs = [
                {
            ...
            for line in f:
                try:
                    ...
                    if (
                        ...
                        or (current_time - violation["timestamp"])
                        <= time_window_seconds
                    ):
...
class RiskControlManager:
    def __init__(
        ...
        self.slippage_tracker = SlippageTracker(
            max_slippage_bps, slippage_cooldown_seconds
        )
    ...
    def save_cooldowns(self, path: Optional[str] = None):
        ...
        fd, temp_path = tempfile.mkstemp(
            dir=state_path.parent, prefix=".cooldowns_", suffix=".tmp"
        )
    ...
    def load_cooldowns(self, path: Optional[str] = None) -> int:
        ...
        try:
            ...
            for cycle_key, cooldown_end in cooldown_data.items():
                if cooldown_end > current_time:
                    cooldown_start = (
                        cooldown_end - self.slippage_tracker.cooldown_seconds
                    )
    ...
    def get_active_cooldowns(self) -> List[Tuple[str, float]]:
        ...
        for cycle_key, start_time in self.slippage_tracker.cooldown_cycles.items():
            remaining = (
                start_time + self.slippage_tracker.cooldown_seconds
            ) - current_time
            if remaining > 0:
    ...
    def extend_cooldown(self, pair: str, seconds: int) -> bool:
        ...
        self.save_cooldowns()
        logger.info(
            f"Extended cooldown for {pair} by {seconds}s (clamped end: {clamped_end})"
        )
        return True
    ...
    def clear_all_cooldowns(self) -> int:
        ...
        self.save_cooldowns()
        logger.info(
            f"Cleared all cooldowns (count: {count}) at {datetime.now().isoformat()}"
        )
        return count
