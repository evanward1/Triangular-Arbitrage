...
class TestRiskControlLogger:
    ...
    def test_log_violation_creates_file(self, temp_log_dir):
        ...
        violation = RiskControlViolation(
            ...
            leg_details=[
                {'leg_index': 0, 'market': 'ETH/BTC', 'side': 'buy', 'latency_ms': 150.5},
                {'leg_index': 1, 'market': 'USDT/ETH', 'side': 'buy', 'latency_ms': 75.2},
                {'leg_index': 2, 'market': 'BTC/USDT', 'side': 'sell', 'latency_ms': 200.3}
            ],
            metadata={}
        )
...
class TestCooldownPersistence:
    ...
    def test_save_cooldowns_atomic_write(self, temp_state_dir):
        ...
        temp_files = [f for f in os.listdir(temp_state_dir) if f.startswith('.cooldowns_')]
